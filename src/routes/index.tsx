...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      { title: translateNow("landing.title") },
      {
        name: "description",
        content: translateNow("landing.metaDescription"),
      },
    ],
  }),
});
          ...
          <div className="relative mx-auto max-w-3xl text-center">
            <div className="mb-6 flex justify-center gap-2">
              ...
              <Sticker color="violet" rotate={3}>
                {t("landing.paymentsSafe")}
              </Sticker>
            </div>
            <h1 className="font-display text-6xl sm:text-8xl leading-[0.9] tracking-tight">
              {t("landing.heroLead")}{" "}
              <span className="text-primary">{t("landing.heroStrong")}</span>
            </h1>
            <p className="mt-6 text-lg text-muted-foreground sm:text-xl">{t("landing.heroText")}</p>
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              {session ? (
                <>
                  ...
                  >
                    {isAdmin(session.role) ? t("landing.adminPanel") : t("landing.toMyCard")}{" "}
                  ...
                  >
                    {canUseBarScanner(session.role)
                      ? t("landing.barScanner")
                      : t("landing.requestDrink")}
                  </Link>
                </>
              ) : (
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-2xl bg-primary px-6 py-3 font-display text-lg tracking-wide text-primary-foreground sticker-lg glow-pink hover:translate-y-[-1px] transition-transform"
                >
                  {t("auth.login")} <ArrowRight className="h-5 w-5" />
                </Link>
              )}
            </div>
            {!session && (
              <p className="mt-4 text-sm text-muted-foreground">{t("landing.loginHint")}</p>
            )}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-4 pb-24">
          <div className="grid gap-6 md:grid-cols-3">
            <div className="rounded-3xl bg-secondary/15 border-2 border-secondary p-6">
              ...
              <p className="text-sm text-muted-foreground mt-1">{t("landing.volunteersBody")}</p>
            </div>
            <div className="rounded-3xl bg-accent/15 border-2 border-accent p-6">
              ...
              <p className="text-sm text-muted-foreground mt-1">{t("landing.barBody")}</p>
            </div>
            <div className="rounded-3xl bg-primary/15 border-2 border-primary p-6">
              <ShieldCheck className="h-8 w-8 text-primary" />
              <h3 className="mt-3 font-display text-2xl">{t("landing.organizationTitle")}</h3>
              <p className="text-sm text-muted-foreground mt-1">{t("landing.organizationBody")}</p>
            </div>
          </div>
        </section>
