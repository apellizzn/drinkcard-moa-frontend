...
import { LanguageProvider, useLanguage } from "@/lib/i18n-react";
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="max-w-md text-center">
        ...
        <h2 className="mt-2 font-display text-2xl">{t("root.notFoundTitle")}</h2>
        <p className="mt-2 text-sm text-muted-foreground">{t("root.notFoundDescription")}</p>
        <div className="mt-6">
          <Link
            to="/"
...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4">
      <div className="max-w-md text-center">
        <h1 className="font-display text-2xl">{t("root.errorTitle")}</h1>
...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      { title: translateNow("root.title") },
      {
        name: "description",
        content: translateNow("landing.metaDescription"),
      },
      { property: "og:title", content: "DrinkCard MOA" },
      { property: "og:description", content: translateNow("root.ogDescription") },
