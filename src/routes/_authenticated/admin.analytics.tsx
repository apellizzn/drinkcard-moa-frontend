...
function AnalyticsPage() {
  const { language, t, intlLocale } = useLanguage();
  ...
  const drinkData = drinkMix(consumedTickets, language);
  const revenueData = revenueByDay(paymentList, intlLocale);
  const ranking = topVolunteers(consumedTickets, userList, t("admin.dashboard.volunteerFallback"));
  const totalRevenue = paymentList.reduce((sum, payment) => sum + (payment.amount ?? 0), 0);
  const peakHour = peak(hourly);
  const topDrink = drinkData[0]?.name ?? t("common.noData");
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">{t("admin.analytics.insightsSubtitle")}</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Insight
          icon={GlassWater}
          label={t("admin.analytics.consumedDrinks")}
        ...
        <Insight
          icon={Clock3}
          label={t("admin.analytics.peakHour")}
          value={ticketList.length ? `${peakHour}h` : "-"}
          tone="amber"
        />
        <Insight icon={Flame} label={t("admin.analytics.topDrink")} value={topDrink} tone="pink" />
      </div>

      <div className="grid gap-6 xl:grid-cols-5">
        <section className="admin-panel p-5 xl:col-span-3">
          <PanelHeader
            title={t("admin.analytics.revenueByDay")}
            subtitle={t("admin.analytics.revenueByDaySubtitle")}
          />
        ...
        <section className="admin-panel p-5 xl:col-span-2">
          <PanelHeader
            title={t("admin.analytics.drinkMix")}
            subtitle={t("admin.analytics.drinkMixSubtitle")}
          />
          <div className="mt-4 grid gap-4 sm:grid-cols-[180px_1fr] xl:grid-cols-1">
            ...
            <div className="space-y-3">
              ...
              {drinkData.length === 0 && <EmptyText>{t("admin.analytics.noDrinkData")}</EmptyText>}
            </div>
          </div>
        </section>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-5">
        <section className="admin-panel p-5 xl:col-span-3">
          <PanelHeader
            title={t("admin.analytics.hourly")}
            subtitle={t("admin.analytics.hourlySubtitle")}
          />
        ...
        <section className="admin-panel p-5 xl:col-span-2">
          <PanelHeader
            title={t("admin.analytics.topVolunteers")}
            subtitle={t("admin.analytics.topVolunteersSubtitle")}
          />
          <div className="mt-5 space-y-4">
            ...
            {ranking.length === 0 && <EmptyText>{t("admin.analytics.noRanking")}</EmptyText>}
          </div>
        </section>
      </div>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <SmallRead
          icon={TrendingUp}
          label={t("admin.analytics.avgTicket")}
          value={paymentList.length ? `${(totalRevenue / paymentList.length).toFixed(2)} €` : "-"}
        />
        <SmallRead
          icon={Users}
          label={t("admin.analytics.volunteersWithConsumption")}
          value={ranking.length}
        />
        <SmallRead
          icon={Clock3}
          label={t("admin.analytics.ticketsAnalyzed")}
      ...
      <div className="h-72 rounded-xl bg-slate-50 p-3">
        {points.length ? (
          <svg
            ...
            role="img"
            aria-label={t("admin.analytics.revenueByDay")}
          >
    ...
    <div className="grid h-56 place-items-center rounded-xl bg-slate-50">
      {total ? (
        <svg
          ...
          className="h-48 w-48"
          role="img"
          aria-label={t("admin.analytics.drinkMix")}
        >
...
function drinkMix(
  ...
  return Object.entries(counts)
    .sort((a, b) => b[1] - a[1])
    .map(([drinkType, count]) => ({
      name: translateDrink(language, drinkType),
      count,
      percent: Math.round((count / total) * 100),
    }));
}

function revenueByDay(payments: AdminPaymentSummary[], intlLocale: string) {
  ...
  payments.forEach((payment) => {
    ...
    const label = date.toLocaleDateString(intlLocale, { day: "2-digit", month: "short" });
