...
import { translateNow, type TranslationKey } from "@/lib/i18n";
...
export const Route = createFileRoute("/_authenticated/admin")({
  beforeLoad: () => {
    ...
    if (!s) throw redirect({ to: "/login", search: { redirect: undefined } });
    if (s.role !== "ADMIN") throw redirect({ to: "/app" });
  },
  component: AdminLayout,
  head: () => ({ meta: [{ title: `${translateNow("nav.adminPanel")} — DrinkCard MOA` }] }),
});
