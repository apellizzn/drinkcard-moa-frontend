...
The app is deployed to **Cloudflare Workers** as a TanStack Start SSR worker. The Worker bundle and a matching `wrangler.json` are produced by Nitro during `vite build` (enabled via `nitro: true` in `vite.config.ts`), and `wrangler deploy` uploads the generated artifact plus the static client assets.

### Requirements

- [Bun](https://bun.sh) installed locally (the project uses `bun.lock`).
...
This script runs `bun run build` and then `wrangler deploy --config dist/server/wrangler.json`. On success, Wrangler prints the live Worker URL, for example:

```text
https://drinkcard-moa-frontend.<account>.workers.dev
```
...
| File | Purpose |
| --- | --- |
| `wrangler.jsonc` | Source-of-truth Worker config: name, `compatibility_date`, `compatibility_flags`, and SSR entry. |
| `dist/server/wrangler.json` | Auto-generated by Nitro at build time. Points to the bundled Worker entry and the `dist/client` static assets. Do not edit by hand. |
| `vite.config.ts` | Sets `nitro: true` so the Cloudflare build runs outside the Lovable sandbox. |

### Adding environment variables or secrets

For non-sensitive build-time variables, add them under `vars` in `wrangler.jsonc`:
